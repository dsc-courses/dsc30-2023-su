...
This approach avoids having to create a new array each time, though it comes at the cost of creating a large array in the constructor even though it won't work if you want to store more than 10 elements. The actual Java implementation of the `ArrayList` class contains a little more code to create a new array double the current capacity when more space is needed, which is why we call this data structure the **dynamic array**.
